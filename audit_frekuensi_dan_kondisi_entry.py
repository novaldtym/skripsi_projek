...
print("="*80)
print("AUDIT FREKUENSI & KONDISI EKSEKUSI SINYAL >= 60%")
print("="*80)
...
rates_h1  = mt5.copy_rates_from_pos(symbol, mt5.TIMEFRAME_H1, 0, 1500)
...
if rates_dxy is not None and len(rates_dxy) > 0:
    ...
    dxy_df = yf.download("DX-Y.NYB", period="30d", interval="15m", progress=False)
...
X_clean = df_m15[features].dropna()
probs = model.predict_proba(X_clean)

prob_up = probs[:, 1] * 100
prob_down = probs[:, 0] * 100
h1_trend = X_clean['Trend_H1_Bull']

valid_mask = ((prob_up >= 60.0) & (h1_trend == 1)) | ((prob_down >= 60.0) & (h1_trend == 0))
valid_trades = X_clean[valid_mask].copy()
valid_trades['close'] = df_m15['close'].reindex(valid_trades.index)
valid_trades['prob_up'] = prob_up[valid_mask]
valid_trades['prob_down'] = prob_down[valid_mask]

print(f"📊 Total Candle Diuji (5.000 Candle Terakhir / ~2 Bulan): {len(X_clean)}")
print(f"🟢 Total Sinyal Valid (>= 60% + Trend Guard): {len(valid_trades)} Sinyal")
print(f"📅 Rata-rata Sinyal Lolos per Minggu: {round(len(valid_trades)/8, 1)} Sinyal/Minggu")

print("\n📋 SAMPLING 10 SINYAL TERAKHIR YANG MEMENUHI SYARAT >= 60% & TREND GUARD H1:")
show_cols = ['close', 'prob_up', 'prob_down', 'FVG_Bull', 'BOS_Bull', 'RSI_M15', 'Trend_H1_Bull']
print(valid_trades[show_cols].tail(10).to_string())
