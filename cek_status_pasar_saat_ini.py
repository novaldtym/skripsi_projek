...
print("="*80)
print("AUDIT REAL-TIME SIGNAL & CONFIDENCE GUARD PASAR SAAT INI")
print("="*80)
...
if not os.path.exists(model_path):
    print("❌ File model_lightgbm_xauusd.pkl tidak ditemukan!")
    sys.exit()
...
if rates_dxy is not None and len(rates_dxy) > 0:
    ...
    dxy_df = yf.download("DX-Y.NYB", period="5d", interval="15m", progress=False)
...
X_latest = df_m15[features].iloc[[-1]]
probs = model.predict_proba(X_latest)[0]
prob_up = probs[1] * 100
prob_down = probs[0] * 100
latest_time = df_m15.index[-1]
latest_price = df_m15['close'].iloc[-1]
h1_trend = "BULLISH 📈" if df_m15['Trend_H1_Bull'].iloc[-1] == 1 else "BEARISH 📉"

print(f"🕒 Timestamp Candle Terbaru : {latest_time}")
print(f"💰 Harga XAUUSD Saat Ini   : ${latest_price:.2f}")
print(f"📈 Tren H1 (Trend Guard)   : {h1_trend}")
print(f"🟢 Probabilitas NAIK (BUY)  : {prob_up:.2f}%")
print(f"🔴 Probabilitas TURUN (SELL): {prob_down:.2f}%")
print("-" * 80)

if prob_up >= 60.0 and df_m15['Trend_H1_Bull'].iloc[-1] == 1:
    print("🚀 KEPUTUSAN: EKSEKUSI BUY! (Syarat 60% + Trend Guard Terpenuhi)")
elif prob_down >= 60.0 and df_m15['Trend_H1_Bull'].iloc[-1] == 0:
    print("🔻 KEPUTUSAN: EKSEKUSI SELL! (Syarat 60% + Trend Guard Terpenuhi)")
else:
    print("🛡️ KEPUTUSAN: HOLD / WAIT & SEE (KONDISI PASAR SIDEWAYS / NOISE / GAMBLING)")
    print(f"   • Alasan: Probabilitas ({max(prob_up, prob_down):.1f}%) masih < 60.0% threshold aman.")
